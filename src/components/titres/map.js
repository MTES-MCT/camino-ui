    ...
    const cluster = L.markerClusterGroup({
      iconCreateFunction(cluster) {
        ...
        return new L.DivIcon({
          html: id.upper,
          className: `py-xs px-s pill h6 mono color-bg cap bold bg-titre-domaine-${id} leaflet-marker-cluster-${size}`,
          iconSize: null,
          iconAnchor: [0, 0]
        })
      },
  ...
  titres.reduce(
    ({ geojsons, markers }, titre) => {
      ...
      const icon = L.divIcon({
        className: `leaflet-marker-camino py-xs px-s pill h6 mono color-bg cap bold border-bg bg-titre-domaine-${domaineId}`,
        html: domaineId,
        iconSize: null,
        iconAnchor: [15.5, 38]
      })
