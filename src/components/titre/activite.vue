      ...
      >
        ...
        <div class="tablet-blobs">
          ...
          >
            ...
            <p class="cap-first">
              {{ activite.contenu && activite.contenu[s.id] && (activite.contenu[s.id][e.id] || activite.contenu[s.id][e.id] === 0) ? e.type === 'checkbox' ? activite.contenu[s.id][e.id].map(c => e.valeurs[c]).join(', ') : activite.contenu[s.id][e.id] : '–' }}
            </p>
          </div>
        </div>
      </div>
